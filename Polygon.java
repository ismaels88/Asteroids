...
//    for (Point p : shape) {
  ...
  */
  ...
  public void paint (Graphics brush){
	  ...
	  int[] xPoints = new int[point.length];
	  int[] yPoints = new int[point.length];
	  for(int i = 0 ; i < point.length;i++){
		
	  	xPoints[i] = (int) point[i].x;
	  	yPoints[i] = (int)point[i].y;
	  	
	  }
	  brush.drawPolygon(xPoints, yPoints, point.length);
	  
	  }
  	public boolean crash(Polygon bang){
  		if(this.contains(bang.position)){
  			return true;
  		}
  		return false;
